...
int cpu_dispatch_alu(const instruction_t* lu, cpu_t* cpu)
{
    ...
    case INC_HLR: {
        // correcteur : no need to extract the carry
        M_EXIT_IF_ERR(alu_add8(&cpu->alu,cpu_read_at_HL(cpu), (uint8_t)1, extract_carry(cpu, lu->opcode)));
    ...
    case INC_R8: {
        M_EXIT_IF_ERR(alu_add8(&cpu->alu, cpu_reg_get(cpu,extract_n3(lu->opcode)), (uint8_t)1, (bit_t)0));
    ...
    case BIT_U3_R8: {
        ...
        // correcteur : just need to set the flag Z, no need to call alu_add8
